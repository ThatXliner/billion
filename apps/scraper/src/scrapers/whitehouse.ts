...
export async function scrapeWhiteHouse() {
  ...
  const crawler = new PlaywrightCrawler({
    async requestHandler({ request, page, log }) {
      ...
      if (request.url.includes('/presidential-actions')) {
        // Wait for action cards to load
        await page.waitForSelector('.presidential-action, article, .entry', { timeout: 10000 }).catch(() => {
          log.warning('No action cards found on page');
        });

        // Extract presidential action links
        const actionLinks = await page.$$eval('a[href*="/presidential-actions/"]', (links) =>
          links
            .map((link) => (link as HTMLAnchorElement).href)
            .filter((href) => href.includes('/presidential-actions/') && !href.endsWith('/presidential-actions/'))
      ...
      else if (request.url.includes('/presidential-actions/') && !request.url.endsWith('/presidential-actions/')) {
        try {
          ...
          const fullText = await page
            .locator('article, .content, .body, main')
            .first()
            .textContent()
            .then((text) => text?.trim())
            .catch(() => undefined);
  ...
  const actionLinksData = data.items.find((item: any) => item.type === 'actionLinks');
